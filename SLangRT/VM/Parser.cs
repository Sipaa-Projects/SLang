...
public class Parser
{
    ...
    public object ParseValue(string valueString)
    {
        ...
        if (trimmedValue.EndsWith("f") || trimmedValue.EndsWith("F"))
            ...
            if (
                double.TryParse(
                    trimmedValue.Remove(trimmedValue.Length - 1, 1),
                    out double dblValue
                )
            )
                return dblValue;
        ...
        {
            SLVariableManager varm = (SLVariableManager)parentRuntime.GetComponentByType(typeof(SLVariableManager));
            var tryVar = varm.GetVarByName(trimmedValue).Value;
            if (tryVar.Value != null)
                return tryVar.Value;
    ...
    public bool IsConditionTrue(string condition)
    {
        ...
        {
            string[] parts = condition.Split(new[] { ">=" }, StringSplitOptions.None);
            return Convert.ToDouble(ParseValue(parts[0].Trim()))
                >= Convert.ToDouble(ParseValue(parts[1].Trim()));
        }
        else if (condition.Contains("<="))
        {
            string[] parts = condition.Split(new[] { "<=" }, StringSplitOptions.None);
        ...
        {
            SLVariableManager varm = (SLVariableManager)parentRuntime.GetComponentByType(typeof(SLVariableManager));
            var tryVar = varm.GetVarByName(condition).Value;
            if (tryVar.Value != null)
            {
                if (tryVar.Value is bool b)
                {
                    return negate ? !b : b;
                }
                else if (tryVar.Value is int i)
                {
